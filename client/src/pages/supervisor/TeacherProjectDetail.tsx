...
import TeacherSprintView from '../../components/teacher/TeacherSprintView';
...
const TeacherProjectDetail: React.FC = () => {
    ...
    const isReviewerView = searchParams.get('role') === 'reviewer';
    const { user } = useAuth();
    const [project, setProject] = useState<Project | null>(null);
    const [activeTab, setActiveTab] = useState<'info' | 'sprints'>('info');
        ...
        <MainLayout>
            <div className={styles.container}>
                ...
                <div style={{
                    ...
                    <button
                        onClick={() => navigate(isReviewerView ? '/teacher/review-projects' : '/teacher/students')}
                        style={{
                    ...
                    {isReviewerView && (
                        <span style={{
                            ...
                            background: 'rgba(255,255,255,0.25)', fontSize: '0.8rem', fontWeight: '600',
                            marginBottom: '1rem', marginLeft: '0.5rem'
                        }}>
                            👨‍⚖️ Vai trò: Giảng viên Phản Biện
                        </span>
                    )}
                ...
                {/* Tab Navigation (Supervisor only) */}
                {!isReviewerView && (
                    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
                        {[
                            { key: 'info' as const, label: '📋 Thông tin', icon: '📋' },
                            { key: 'sprints' as const, label: '🏃 Sprint Planning', icon: '🏃' },
                        ].map(tab => (
                            <button
                                key={tab.key}
                                onClick={() => setActiveTab(tab.key)}
                                style={{
                                    padding: '0.75rem 1.5rem', borderRadius: '0.75rem', border: '2px solid',
                ...
                {/* Sprint Tab Content */}
                {activeTab === 'sprints' && !isReviewerView && (
                    <div style={{ marginBottom: '2rem' }}>
                        <TeacherSprintView projectId={project.id} />
                    </div>
                )}

                {/* Info Tab Content */}
                {(activeTab === 'info' || isReviewerView) && (
                    <>
                        ...
                        {/* Feedback Section - Only for Supervisor */}
                        {!isReviewerView && (
                            <div style={{
                                background: 'white',
                                borderRadius: '1rem',
                        ...
                        {/* Progress Reports Section - Only for Supervisor */}
                        {!isReviewerView && (
                            <div style={{
                                background: 'white',
                                borderRadius: '1rem',
